...
另外多数计算机不是将程序和系统在外部编译成机器码并导入一个ROM来使用，而是在运行过程中随时进行编译工作。这就很考验开发语言的编译器是否能写的比较简洁。每翻译一条高级程序语句到底需要动用多少代码量，占用多少内存空间。所以更亲近人类的编程语言在这里就没有什么优势，因为语法复杂度高，导致编译时不得不来回阅读上下文，增加了不少工作量。所以所有高级语言都可以写操作系统也只是一个理论上的说法。现实中只有那几个语言被用来开发系统是有道理的。
