...
这里面系统（OS）也是一堆类（class）是最让我感觉意外的。
...
对于本课程这套简单的只执行一个程序的计算机来说。有了以上这四条协议，在语法中，在设计各级编译器时，在编写应用程序，系统时，都要考虑各部分需承担的协议内容。如此可以保证不论写一个什么程序，经过complier，VM translator，再转成机器码，导入ROM里以后，一按下开机键，就会自动启动系统并执行这个程序。

这四种协议是不是唯一的方案呢？显然不是。首先它适用的场景是当前这台简单的计算机。当计算机的功能变得复杂，要执行的程序不再是单一程序，这四条协议就可能不合适了。即便是同样一台计算机，是否非要经过三层翻译，booting code是不是只能在中间语言里靠虚拟机翻译器添加？也显然不是这样，总是有其他协议方案的。但一种协商结果，可能是综合起来看最舒适最合理的的。
让整个计算机系统能顺畅运行的一个特点，就是一切靠商量，商量定以后，各方都必须按协议办事。输入输出设备有驱动，传输有协议，硬件厂商有如IEEE协会这种组织，想必都是为进行“商量”。
