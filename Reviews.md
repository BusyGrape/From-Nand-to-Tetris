...
如何让，一个一次只能将两个数据进行运算的芯片，和两串顺序排列的寄存器，帮我们实现复杂的工作呢？

比如：
- 重复特定的计算步骤
- 如果有三个或者更多的输入（变量），如果部分变量还有多个共享使用者，如果需要输入的变量是某个计算步骤的计算结果
- 计算过程产生了需要暂存的中间结果，而且有不止一个，或者要间隔很多计算步骤以后才会用到中间结果
...
包括：
- 将内存切分成几个功能区域（segment），来分别进行指针数据、不同类型的变量、计算过程，输入输出数据的存储
- 用指针（pointer），来记录内存区域的划分（每个区域的起始位置）以便能准确找到特定区域里的数据
...
- 堆（heap）分配，为连续数据和对象数据划拨出所需要的存储空间

这套精巧的方法同时也是所有高级编程语言共有功能的基础：
- 定义和调用公式（function/method/construction）
- 创建和导入类（class）、对象（object）
- 定义某些实际是是数据串的数据类型（array/list/string...）
- 打包和执行程序
- 文件（本质上是数据串）

设立中间语言和虚拟机环节并不是必须，但是它可以专注实现这套内存使用规则，还能承担跨芯片平台编译机器码的工作，从而大大减轻高级语言的编译器设计负担。
