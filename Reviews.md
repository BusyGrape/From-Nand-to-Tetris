...
## 芯片做什么事情？
 ...
 - 做什么运算
   ...
   本课程的ALU有一个全加器，所以它让x和y进行**加减运算**，或者让x、y中任意一个和0、1之间做加减运算；
   另外ALU中的16位与、或、非门，可以支持x和y之间做**与，或，非**的逻辑运算，以及x、y之中任意一个和0、1之间做与、或、非的逻辑运算。

 - 程序控制

   输入什么数据，做什么样的运算，由程序指令来控制的。
   程序指令加载在ROM里，CPU会从第一个ROM[0]的位置开始一条接一条读取。
   遇到A类指令，就把ROM[PC]的数据放到A寄存器里，
   遇到C类指令，就根据指令进行两个输入数据的运算，并且根据指令把计算结果输出到相应地方。
