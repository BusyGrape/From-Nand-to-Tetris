...
这些东西都有各自的特征。比如表格的行数列数和每个格里的数据；方块的边长，位置和颜色。同时每一个具体物件有属于它的独立的数据，比如一个表是3*3，另一个是5*2；我们在屏幕上画了10个大小不同的方快。
...
要知道芯片能做的事情始终只是进行逻辑判断，实现跳转步骤，对指定位置提取或存入数据，以及做非常基础的数学运算。我们巧妙地将这几个基础功能进行组合，就可以实现复杂一些的事情。每一条指令，都是上一层（低级）语言的若干条指令的集合。我们把常用的组合固定下来，并给他命名（俗称造轮子的过程）。学习高级语言的一大块工作是在熟悉它提供了多少的轮子。而编程，则主要是根据要实现的事情，分析并挑选出适合的轮子，然后把它们再组合成一个更大的结构的过程。对已有公式的熟悉程度，对组合利用公式的方案的熟悉程度，非常影响一个人编程的效率。
