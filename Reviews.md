...
- 用指针（pointer），来记录区域的划分（每个区域的起始位置）以便能准确找到特定区域里的数据
- 动态使用栈（stack），巧妙地实现了多个工作交叉进行且独立保存各自进度
- 堆（heap）分配，为连续数据和对象数据划拨出所需要的存储空间

这套精巧的方法同时也是所有高级编程语言可以实现：定义和调用公式（function/method/construction）；创建和导入类（class）、对象（object）、定义某些实际是是数据串的数据类型（array/list/string...）；打包和执行程序的基础。

设立中间语言和虚拟机环节，除了可以专注实现这套内存使用规则，还能让这个环节承担跨芯片平台编译机器码的工作。大大减轻高级语言的编译器设计负担。
